...
static void TestPailierC$Boot$booted(void );
...
#line 122
message_t * amsg);
# 19 "/opt/tinyos-2.1.1/tos/interfaces/Pailier.nc"
static uint64_t PailierC$Pailier$decrypt(uint64_t in, uint64_t g, uint64_t h, uint64_t p, uint64_t q, uint64_t out);
#line 11
static uint64_t PailierC$Pailier$encrypt(uint64_t in, uint64_t g, uint64_t n, uint64_t out);
...
static uint64_t TestPailierC$Pailier$decrypt(uint64_t in, uint64_t g, uint64_t h, uint64_t p, uint64_t q, uint64_t out);
#line 11
static uint64_t TestPailierC$Pailier$encrypt(uint64_t in, uint64_t g, uint64_t n, uint64_t out);
# 13 "TestPailierC.nc"
static uint64_t TestPailierC$gcd(uint64_t a, uint64_t b);
#line 38
static inline uint64_t TestPailierC$lcm(uint64_t a, uint64_t b);
...
static inline void TestPailierC$Boot$booted(void );
# 51 "/opt/tinyos-2.1.1/tos/interfaces/Init.nc"
...
# 16 "PailierC.nc"
static inline uint64_t PailierC$gcd(uint64_t a, uint64_t b);
#line 43
static inline uint64_t PailierC$lcm(uint64_t a, uint64_t b);
#line 134
static uint64_t PailierC$Pailier$encrypt(uint64_t in, uint64_t g, uint64_t n, uint64_t out);
#line 158
static uint64_t PailierC$Pailier$decrypt(uint64_t de, uint64_t g, uint64_t h, uint64_t p, uint64_t q, uint64_t out);
...
# 67 "/opt/tinyos-2.1.1/tos/interfaces/Receive.nc"
inline static message_t * TossimActiveMessageC$Snoop$receive(am_id_t arg_0x4085f7f0, message_t * msg, void * payload, uint8_t len){
#line 67
  nx_struct message_t *__nesc_result;
#line 67

#line 67
    __nesc_result = TossimActiveMessageC$Snoop$default$receive(arg_0x4085f7f0, msg, payload, len);
#line 67
...
# 67 "/opt/tinyos-2.1.1/tos/interfaces/Receive.nc"
inline static message_t * TossimActiveMessageC$Receive$receive(am_id_t arg_0x4085f158, message_t * msg, void * payload, uint8_t len){
#line 67
  nx_struct message_t *__nesc_result;
#line 67

#line 67
    __nesc_result = TossimActiveMessageC$Receive$default$receive(arg_0x4085f158, msg, payload, len);
#line 67
...
# 19 "/opt/tinyos-2.1.1/tos/interfaces/Pailier.nc"
inline static uint64_t TestPailierC$Pailier$decrypt(uint64_t in, uint64_t g, uint64_t h, uint64_t p, uint64_t q, uint64_t out){
#line 19
  unsigned long long __nesc_result;
#line 19

#line 19
  __nesc_result = PailierC$Pailier$decrypt(in, g, h, p, q, out);
#line 19
...
#line 19
#line 11
inline static uint64_t TestPailierC$Pailier$encrypt(uint64_t in, uint64_t g, uint64_t n, uint64_t out){
#line 11
  unsigned long long __nesc_result;
#line 11

#line 11
  __nesc_result = PailierC$Pailier$encrypt(in, g, n, out);
#line 11
...
#line 38
{
  uint64_t u = a * b / TestPailierC$gcd(a, b);
...
#line 60
  ...
  if (TestPailierC$gcd(p * q, (p - 1) * (q - 1)) == 1) {
      sim_log_debug(46U, "Boot", "**Condition for gcd of n and -1 is true \n");


      l = TestPailierC$lcm(p - 1, q - 1);
      sim_log_debug(47U, "Boot", "%ld\n", l);
      e = 2;
      *c = pow(e, l);
      y = *c % squ;
      ...
      if (TestPailierC$gcd(a, n) == 1) {
          sim_log_debug(48U, "Boot", "**Condition for gcd of L and n is true \n");
          g = e;
          ...
          switch (a1) 
            {
              case 1: 
                ...
                {
                  sim_log_debug(49U, "Boot", "Enter another number whic is coprime with n \n");
                  break;
                }
                  ...
                  for (i = 1; i <= n - 1; i++) 
                    {
                      ...
                      if (inv == 1) 
                        {
                          h = i;
                          sim_log_debug(50U, "Boot", "**tHE INVERSE OF z1 IS %ld \n ", i);
                        }
                    }
                  break;
                }
          ...
          out1 = TestPailierC$Pailier$encrypt(in1, g, n, out1);
          out2 = TestPailierC$Pailier$encrypt(in2, g, n, out2);
          sim_log_debug(51U, "Boot", " encryption in in1 %ld \n", out1);
          sim_log_debug(52U, "Boot", "encryption in in2 %ld \n", out2);

          h1 = TestPailierC$Pailier$decrypt(out1, g, h, p, q, x);
          d = TestPailierC$Pailier$decrypt(out2, g, h, p, q, f);
          sim_log_debug(53U, "Boot", "decryption is %ld \n", h1);
          sim_log_debug(54U, "Boot", "decryption is %ld \n", d);
        }
    }
  else 
    ...
    {
      sim_log_debug(55U, "Boot", "**Check values**\n\n");
    }
}
...
#line 43
{
  uint64_t u = a * b / PailierC$gcd(a, b);
...
#line 138
  s = (uint64_t *)malloc(sizeof(uint64_t ) * 10);
  m = (uint64_t *)malloc(sizeof(uint64_t ) * 5);
...
#line 162
  ...
  m1 = (uint64_t *)malloc(sizeof(uint64_t ) * 6);
